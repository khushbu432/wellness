...
useEffect(() =>{
  fetch(`https://logink123.herokuapp.com/drug/findDrug?${option}=${input}`, requestOptions)
  .then(response => response.json())
  .then((response) => {
    if (response.success === true) {
      ReactSession.set("drug",response.drug)
      const drug=ReactSession.get("drug")
      setResult(response.drug)
    }
    })
...
onMouseEnter={(e) => ReactSession.set("deletedrug",curElem._id)} onClick={deletedrug}>Delete</button> </a> </td>
                            </tr>
